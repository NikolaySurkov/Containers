...
TEST(DEFAULT_CONSTRUCTOR_RBTREE, test_1) {
  ns::RBTree<NotDefaultConstructor> a;
  EXPECT_EQ(a.Size(), 0U);
  a.Insert({"1", "2", "3"});
  EXPECT_EQ(a.Size(), 1U);
  a.Insert({"9", "8", "7"});
...
TEST(CONSTRUCTOR_RBTREE, test_2) {
  ns::RBTree<NotDefaultConstructor> a(NotDefaultConstructor{"a", "b", "c"});
